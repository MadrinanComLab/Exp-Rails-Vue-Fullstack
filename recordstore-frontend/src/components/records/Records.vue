  ...
  <div class="max-w-lg m-auto py-10">
    ...
    <form @submit.prevent="addRecord">
      <div class="mb-6">
        ...
        <input type="text" id="record_title" class="bg-gray-200 block p-1 w-full rounded-sm" autofocus autocomplete="off" placeholder="Type an record name" v-model="newRecord.title">
      </div>

      <div class="mb-6">
        <label for="record_year">Year:</label>
        <input type="text" id="record_year" class="bg-gray-200 block p-1 w-full rounded-sm" autofocus autocomplete="off" placeholder="Type an record year" v-model="newRecord.year">
      </div>

      <div class="mb-6">
        <label for="artist">Artist:</label>
        <select id="artist" class="bg-gray-200 block p-1 w-full rounded-sm" v-model="newRecord.artist">
        ...
        <p class="pt-4">Don't see an artist? <router-link to="/artists">Create One</router-link></p>
      </div>
        ...
        <li class="py-4" v-for="record in records" :key="record.id" :record="record">
          <div class="flex items-center justify-between flex-wrap">
            <div class="flex-1 flex justify-between flex-wrap pr-4">
              ...
              <button class="bg-transparent text-sm hover:bg-blue-700 hover:text-white text-blue-500 border border-blue-500 no-underline font-bold py-2 px-4 mr-2 rounded" @click.prevent="editRecord(record)">Edit</button>
              <button class="bg-transparent text-sm hover:bg-red-700 hover:text-white text-red-500 border border-red-500 no-underline font-bold py-2 px-4 mr-2 rounded" @click.prevent="removeRecord(record)">Delete</button>
            </div>
            ...
            <div v-if="record == editedRecord">
              <form @submit.prevent="updateRecord(record)">
                <div class="mb-6 p-4 bg-white rounded border border-gray-100 mt-4">
                  <div class="mb-6">
                    <label>Title:</label>
                    <input class="bg-gray-200 block p-1 w-full rounded-sm" v-model="record.title">
                  </div>

                  <div class="mb-6">
                    <label>Year:</label>
                    <input class="bg-gray-200 block p-1 w-full rounded-sm" v-model="record.year">
                  </div>

                  <div class="mb-6">
                    <label>Artist:</label>
                    <select id="artist_update" class="bg-gray-200 block p-1 w-full rounded-sm" v-model="record.artist">
                      <option disabled value="">Select an artist</option>
                      <option :value="artist.id" v-for="artist in artists" :key="artist.id">{{ artist.name }}</option>
                    </select>
                  </div>

                  <input type="submit" value="Update" class="bg-transparent text-sm hover:bg-blue-700 hover:text-white text-blue-500 border border-blue-500 no-underline font-bold py-2 px-4 mr-2 rounded">
                </div>
              </form>
            </div>
          </div>
